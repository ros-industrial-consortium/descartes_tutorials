// This tutorial is exactly the same as tutorial2 with the exception that it uses the opw kinematics
// package instead of the IKFast model.
...
int main(int argc, char** argv)
{
  ...
  // DIFFERENCE: This is the only difference between tutorial2 and this one

  // In Descartes, the "Robot Model" is repsonsible for solving forward and inverse kinematics, and for doing collision
  // checking. In this particular tutorial, we make use of a model that uses a kinematics solver for robot's with
  // "Ortho-parallel Basis and a Spherical Wrist" (https://github.com/Jmeyer1292/opw_kinematics). See the package for
  // more details.

  // The Descartes OPW model requires that you set a few parameters:
  //   1. The "base frame" of the kinematic solver - this is the base of the robot's kinematic chain
  //   2. The "tool frame" of the kinematic solver - the tool frame of the kinematic solver, usually tool0
  // Note that these may be different than those frames passed to "initialize()". These need to be the
  // robot's frames, not tool frames attached.
  const std::string kin_base_frame = "base_link";
  const std::string kin_tool_frame = "tool0";

  // Load the IRB2400 robot model parameters
  const opw_kinematics::Parameters<double> kin_params = opw_kinematics::makeIrb2400_10<double>();

  // Create the robot model itself
  descartes_core::RobotModelPtr model (new descartes_opw_model::OPWMoveitStateAdapter(kin_params, kin_base_frame,
                                                                                      kin_tool_frame));
  // END OF DIFFERENCE
